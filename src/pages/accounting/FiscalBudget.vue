  ...
  <q-page class="q-pa-md">
    <div class="row q-mb-md items-center justify-end" style="width: 100%">
      <q-select v-model="visibleColumns" multiple outlined dense options-dense :options="columns.map((col) => col.name)"
        ...
        class="q-mr-sm" />
      <q-btn color="primary" icon="add" label="New Budget" @click="openModal()" />
    </div>

    <!-- Budget Table -->
    <q-table :rows="budgets" :columns="columns" row-key="ID" :loading="loading" flat bordered
    ...
    <q-dialog v-model="showModal" persistent>
      <q-card style="width: 50vw; max-width: 90vw; max-height: 90vh; display: flex; flex-direction: column;">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">{{ editingBudget ? 'Edit Budget' : 'New Budget' }}</div>
          <q-space />
    ...
    <q-dialog v-model="showDeleteDialog">
      <q-card>
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm">Are you sure you want to delete this budget entry?</span>
        </q-card-section>
...
const columns: QTableColumn[] = [
  ...
  },
  {
    name: 'CREATED_AT',
    label: 'Created At',
    field: (row: Budget) => formatDate(row.CREATED_AT),
    sortable: true,
    align: 'left' as const,
  },
  {
...
const formData = ref<Partial<Budget>>({
  AMOUNT: 0,
  REMARKS: null,
})
...
const openModal = (budget: Budget | null = null) => {
  ...
  if (budget) {
    ...
    selectedAccount.value = accounts.value.find(a => a.AC_CODE === budget.AC_CODE) || null
  } else {
    formData.value = {
      AMOUNT: 0,
      REMARKS: null,
...
const saveBudget = async () => {
  try {
    ...
    if (editingBudget.value) {
      ...
      const { error } = await supabase.from('BUDGET').insert(saveData)
...
const deleteBudget = async () => {
  try {
    loading.value = true
    if (!budgetToDelete.value?.ID) return

    const { error } = await supabase
      .from('BUDGET')
      .delete()
      .eq('ID', budgetToDelete.value.ID)

    if (error) throw error
    $q.notify({ type: 'positive', message: 'Budget deleted successfully' })
    await fetchBudgets()
  } catch (err) {
    console.error(err)
    $q.notify({
